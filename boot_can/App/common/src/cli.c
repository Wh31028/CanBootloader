...
bool cliIsBusy(void) { return cli_node.is_busy; }
...
uint8_t cliGetPort(void) { return cli_node.ch; }
...
uint32_t cliAvailable(void) { return uartAvailable(cli_node.ch); }

uint8_t cliRead(void) { return uartRead(cli_node.ch); }
...
bool cliUpdate(cli_t *p_cli, uint8_t rx_data) {
  ...
  if (p_cli->state == CLI_RX_IDLE) {
    ...
    case CLI_KEY_BACK:
      // 1. 지울 글자가 있을 때만 실행
      if (line->cursor > 0) {
        // 2. 중간에서 지우는 경우 (커서가 끝이 아닐 때) 버퍼 당기기
        if (line->cursor < line->count) {
          for (int i = line->cursor; i < line->count; i++) {
        ...
        line->count--;
        line->cursor--;
        line->buf[line->count] = 0; // 버퍼의 끝을 항상 NULL로 닫아줌

        // 4. 터미널 화면 제어
        // \b: 커서 뒤로, \x1B[1P: 현재 칸 글자 삭제하며 뒤쪽 글자 당겨오기
        uartPrintf(p_cli->ch, "\b\x1B[1P");
      }
      break;
...
void cliMoveUp(uint8_t y) { cliPrintf("\x1B[%dA", y); }

void cliMoveDown(uint8_t y) { cliPrintf("\x1B[%dB", y); }
